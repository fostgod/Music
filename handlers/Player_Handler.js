      ...
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `_ ${emoji.ERROR} บอทออกจากห้องเสียงแล้ว_`
            )
            .setFooter({ text: ee.footertext, iconURL: ee.footericon }),
        ],
      });
      ...
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setImage(
            `https://media.discordapp.net/attachments/1046801686225760339/1046990155887693824/20221117_150639.jpg`
          )
            .setDescription(
              ` การเล่นจบแล้ว \n`
            )
          ...
          case "playp":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${emoji.ERROR} คุณต้องเข้าห้องเสียงที่มี kuro okami **`
                );
    ...
    if (message.channel.id === channel.id) {
      if (message.author.bot) {
        ...
        if (!voiceChannel) {
          ...
          return send(message, `คุณต้องเข้าห้องเสียงที่มี kuro okami`);
        } else {
